<template>
    <div class="w-60 pr-3 h-full overflow-y-auto bg-[#212121]" id="yt-sidebar-menu">
        <ul class="py-3 border-b border-[#323232]" v-for="(ul, index) in menus" :key="index">
            <p class="flex items-center pl-5 pt-3 text-sm font-medium text-[#8a8a8a] transition duration-75 group" v-show="ul.subject !== ''">
                {{ ul.subject }}
            </p>
            <template v-if="ul.isSubscription === true">
                <li v-for="(li, index) in ul.lis" :key="index">
                    <a href="#" class="flex items-center py-3 pl-5 text-sm text-white hover:bg-[#363636]">
                        <span class="flex rounded-full">
                            <img class="w-6 h-6 rounded-full" :src="li.img" alt="Subscription Brand" />
                        </span>
                        <span class="flex w-32 text-left leading-none truncate ml-5">{{ li.title }}</span>
            ...
            <template v-else>
                <li v-for="(li, index) in ul.lis" :key="index">
                    <a href="#" class="flex items-center py-3 pl-5 text-sm text-white hover:bg-[#363636]">
                        <span class="w-6 h-6 fill-white" v-html="li.svg"> </span>
                        <span class="flex w-32 text-left leading-none truncate ml-5">{{ li.title }}</span>
                    </a>
                </li>
            </template>
                ...
                lis: [
                    {
                        ...
                        title: "首頁",
                        svg: `  
                        <svg class="yt-icon w-full h-full block" viewBox="0 0 24 24" preserveAspectRatio="xMidYMid meet" focusable="false">
                ...
                lis: [
                    {
                        ...
                        svg: `  
                        <svg class="yt-icon w-full h-full block" viewBox="0 0 24 24" preserveAspectRatio="xMidYMid meet" focusable="false">
                            <g><path d="M11,7l6,3.5L11,14V7L11,7z M18,20H4V6H3v15h15V20z M21,18H6V3h15V18z M7,17h13V4H7V17z"></path></g>
                        </svg> `,
                    },
                    {
                        id: "History",
                    ...
                    },
                    {
                        ...
                        svg: `  
                        <svg class="yt-icon w-full h-full block" viewBox="0 0 24 24" preserveAspectRatio="xMidYMid meet" focusable="false">
                            <g><path d="M10,8l6,4l-6,4V8L10,8z M21,3v18H3V3H21z M20,4H4v16h16V4z"></path></g>
                        </svg> `,
                    },
                    {
                        id: "Watch-Later",
